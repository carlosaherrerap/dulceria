-- Esquema para la base de datos del bot de WhatsApp (PostgreSQL)
...
-- Propiedades de productos (ej. sabor, relleno)
...
-- Plantillas JSON usadas por el bot para flujos dinámicos
...
-- Medios (imágenes, PDFs, etc.) referenciados por el bot
CREATE TABLE medios (
    id VARCHAR(50) PRIMARY KEY,
...
CREATE TABLE preguntas_frecuentes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    -- Categoría del reclamo/consulta: 'reclamo', 'consulta', 'pago', 'entrega', etc.
    categoria VARCHAR(50) NOT NULL DEFAULT 'general',
    -- Pregunta o frase típica del cliente
    pregunta TEXT NOT NULL,
    -- Palabras clave separadas por coma para que el bot pueda hacer matching
    palabras_clave TEXT,
    -- Respuesta automática que el bot enviará
    respuesta TEXT NOT NULL,
    -- Si está activa para que el bot la use
    activo BOOLEAN DEFAULT TRUE,
    creado_en TIMESTAMP WITH TIME ZONE DEFAULT now(),
    actualizado_en TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
CREATE TABLE reclamos (
    ...
    -- Texto del reclamo enviado por el cliente
    descripcion TEXT NOT NULL,
    -- Estado: 'PENDIENTE', 'EN_REVISION', 'RESUELTO', 'DERIVADO'
    estado VARCHAR(20) NOT NULL DEFAULT 'PENDIENTE',
    -- Si el bot encontró respuesta automática, se guarda aquí
    respuesta_automatica TEXT,
    -- Si fue derivado, el empleado que lo atiende
    empleado_id UUID REFERENCES empleados(id),
    -- Respuesta final del empleado
...
-- Ej: "Es el cumpleaños de mi hermano" → bot sugiere combos de esta tabla
-- -------------------------------------------------------
CREATE TABLE sugerencias_mixtas (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    -- Evento/ocasión: 'cumpleaños', 'aniversario', 'día de la madre', etc.
    ocasion VARCHAR(100) NOT NULL,
    -- Palabras clave para detectar la ocasión desde el mensaje del cliente
    palabras_clave TEXT,
    -- Descripción del combo sugerido
    descripcion TEXT NOT NULL,
    -- IDs de presentaciones incluidas en la sugerencia (formato JSON array)
    presentaciones_ids JSON,
    -- Precio total estimado del combo
